...
<style type="text/css">
    ...
    .main-content .right { width: 40%; box-shadow: 1px 1px 10px #999; border-radius: 0px; margin-left: 1%; max-height: 500px;overflow: scroll; }
    .btn { width: 30%; }
			...
			<form action="{{route('admin.product.edit',$data->id)}}" method="post" enctype="multipart/form-data">
				...
				<table>
					...
					<tr ondblclick='edit("type")'>
						<th>Loại</th>
						<td>
							<select id='type' name='type_code' class="form-control selectpicker select" data-style='btn-succes' title="{{$data->type->name}}">
								@foreach ($types as $type)
									<option value="{{$type->code}}">{{$type->name}}</option>
								@endforeach
							</select>
							<!-- <input type="text" id="type" name='type_code'placeholder="{{$data->type->name}}" disabled autocomplete="off"> -->
						</td>
					</tr>
					...
					<tr ondblclick='edit("price")'>
						<th>Giá</th>
						<td>
							<input type="text" id="price" name='price'placeholder="{{$data->price}} đ" disabled autocomplete="off">
						</td>
					</tr>
